...
import { InvestigationalUseDialog, Onboarding, ResizablePanelGroup, ResizablePanel, ResizableHandle } from '@ohif/ui-next';
...
function DentalViewerLayout({
  ...
  useEffect(() => {
    document.body.classList.add('bg-background', 'overflow-hidden');
    return () => {
      document.body.classList.remove('bg-background', 'overflow-hidden');
    };
  }, []);
  ...
  const getComponent = id => {
    ...
    if (!entry?.component) {
      throw new Error(`${id} is not valid for an extension module.`);
    }
    return { entry };
  };
  ...
  useEffect(() => {
    const { unsubscribe } = panelService.subscribe(panelService.EVENTS.PANELS_CHANGED, ({ options }) => {
      setHasLeftPanels(hasPanels('left'));
      setHasRightPanels(hasPanels('right'));
      if (options?.leftPanelClosed !== undefined) setLeftPanelClosed(options.leftPanelClosed);
      if (options?.rightPanelClosed !== undefined) setRightPanelClosed(options.rightPanelClosed);
    });
    return () => unsubscribe();
  ...
  return (
    <div>
      <PracticeHeader
        ...
        appConfig={appConfig}
        practiceName={appConfig.dentalPracticeName || 'Bright Smile Dental'}
      />
      <div
